...
public class OpenERPCustomerParameterMapper {
  ...
  public OpenERPRequest mapCustomerParams(Customer customer, String operation) {
    ...
    Parameter ref = new Parameter(
      OPENERP_CUSTOMER_REF,
      customer.getRef(),
      "string"
    );
    Parameter village = new Parameter(
      OPENERP_CUSTOMER_VILLAGE,
      customer.getVillage(),
      "string"
    );
    Parameter sex = new Parameter(
      OPENERP_CUSTOMER_SEX,
      customer.getSex(),
      "string"
    );

    List<Parameter> parameters = Arrays.asList(name, ref, village, sex);
    String resource = CUSTOMER_RESOURCE;
    OpenERPRequest request = new OpenERPRequest(
      resource,
      operation,
      parameters
    );
