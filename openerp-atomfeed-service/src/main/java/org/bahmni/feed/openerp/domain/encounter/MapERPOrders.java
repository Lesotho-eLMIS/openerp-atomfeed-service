...
public class MapERPOrders extends OpenMRSEncounterEvent {
  ...
  public List<Parameter> getParameters(
    String eventId,
    String feedURIForLastReadEntry,
    String feedURI
  ) throws IOException {
    ...
    parameters.add(
      createParameter("customer_id", openMRSEncounter.getPatientId(), "string")
    );
    parameters.add(
      createParameter(
        "encounter_id",
    ...
    parameters.add(
      createParameter(
        "feed_uri_for_last_read_entry",
        feedURIForLastReadEntry,
        "string"
      )
    );
    ...
    parameters.add(
      createParameter(
        "locationName",
        openMRSEncounter.getLocationName(),
        "string"
      )
    );
    return parameters;
  ...
  private String mapOpenERPOrders() throws IOException {
    OpenERPOrders openERPOrders = new OpenERPOrders(
      openMRSEncounter.getEncounterUuid()
    );
    ...
    String providerName = providers.size() != 0
    ...
    for (OpenMRSDrugOrder drugOrder : openMRSEncounter.getDrugOrders()) {
      ...
      observations
        ...
        .filter(
          o ->
            o.getOrderUuid() != null &&
            o.getOrderUuid().equals(openERPOrder.getOrderId())
        )
        .forEach(o -> {
          if (
            (o.getConcept() != null) &&
      ...
      //new fields to getFullDosage
      openERPOrder.setDose(
        drugOrder.getDosingInstructions().getDose() != null
          ? drugOrder.getDosingInstructions().getDose().toString()
          : null
      );
      openERPOrder.setDoseUnits(
        drugOrder.getDosingInstructions().getDoseUnits()
      );
      if (drugOrder.getAdministrationInstructions() != null) {
        openERPOrder.setAdministrationInstructions(
          drugOrder.getAdministrationInstructions()
        );
      }

      if (drugOrder.getDuration() != null) {
        openERPOrder.setDuration(drugOrder.getDuration());
      }

      if (drugOrder.getDurationUnits() != null) {
        openERPOrder.setDurationUnits(drugOrder.getDurationUnits());
      }

      if (drugOrder.getNumberOfRefills() != null) {
        openERPOrder.setNumRefills(drugOrder.getNumberOfRefills());
      }

      if (drugOrder.getDosingInstructions().getAsNeeded() != null) {
        openERPOrder.setAsNeeded(
          drugOrder.getDosingInstructions().getAsNeeded().toString()
        );
      }
      openERPOrder.setFrequency(drugOrder.getFrequency());
      openERPOrder.setRoute(drugOrder.getRoute());
      openERPOrder.setAdministrationInstructions(
        drugOrder.getAdministrationInstructions()
      );
      openERPOrder.setDuration(drugOrder.getDuration());
    ...
    return ObjectMapperRepository.objectMapper.writeValueAsString(
      openERPOrders
    );
  }

  private void addPreviousDrugOrder(
    OpenERPOrder openERPOrder,
    OpenERPOrders openERPOrders
  ) {
    if (
      "REVISE".equals(openERPOrder.getAction()) &&
      !isPreviousOrderFound(openERPOrder, openERPOrders)
    ) {
      OpenERPOrder previousOrder = fetchPreviousDrugOrder(
        openERPOrder.getPreviousOrderId()
      );
  ...
  private boolean isPreviousOrderFound(
    OpenERPOrder openERPOrder,
    OpenERPOrders openERPOrders
  ) {
    ...
    for (OpenERPOrder existingOrder : existingOrders) {
      if (
        existingOrder.getOrderId().equals(openERPOrder.getPreviousOrderId())
      ) {
        return true;
      }
    }
  ...
  private OpenERPOrder fetchPreviousDrugOrder(String previousOrderId) {
    String apiUrl =
      openERPAtomFeedProperties.getDrugOrderUri() + "/" + previousOrderId;
    final ObjectMapper mapper = new ObjectMapper();
    try {
      String response = openMRSWebClient.get(URI.create(apiUrl));
      OpenMRSDrugOrder previousOrder = mapper.readValue(
        response,
        OpenMRSDrugOrder.class
      );
      ...
      //new fields
      order.setFrequency(previousOrder.getFrequency());
      order.setRoute(previousOrder.getRoute());
      order.setAdministrationInstructions(
        previousOrder.getDosingInstructionsText()
      );

      order.setDuration(previousOrder.getDuration());
      order.setDurationUnits(previousOrder.getDurationUnits());
      order.setNumRefills(previousOrder.getNumberOfRefills());
      order.setAsNeeded(
        previousOrder.getDosingInstructions().getAsNeeded() != null
          ? previousOrder.getDosingInstructions().getAsNeeded().toString()
          : "false"
      );
      order.setAdministrationInstructions(
        previousOrder.getAdministrationInstructions()
      );
      return order;
    } catch (Exception e) {
      logger.error(
        "Error occurred while fetching previous drug order: " + e.getMessage()
      );
  ...
  private Map<String, OpenERPOrder> findLatestOrder(
    OpenERPOrder openERPOrder,
    Map<String, OpenERPOrder> latestOrders
  ) {
    String productId = openERPOrder.getProductId();
    if (
      !latestOrders.containsKey(productId) ||
      latestOrders
        .get(productId)
        .getDateCreated()
        .before(openERPOrder.getDateCreated())
    ) {
  ...
  private String getVisitType() {
    for (VisitAttributes visitAttribute : openMRSVisit.getAttributes()) {
      if (
        visitAttribute.getAttributeType().getDisplay().equals("Visit Status")
      ) {
